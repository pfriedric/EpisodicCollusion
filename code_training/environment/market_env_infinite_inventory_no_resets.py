...
# region ENVIRONMENT
class MarketEnvInfiniteInventoryInfiniteEpisode(environment.Environment):
    """This is meant to be used in the infinite episode setting but without inventory constraints.
    2 changes to regular MarketEnv:
    - this one will never decrement the inventory (like InfiniteInventory).
    - in addition to InfiniteInventory, this never returns 'done' and is meant for one infinite episode.
    """
    ...
    def __init__(self, num_agents: int, num_actions: int, time_horizon: int):
        ...
        def _step(
            ...
            done = t + 10000000000 >= params.time_horizon  # check if we're done
            # key, _ = jax.random.split(key, 2)  # split the key, just in case
            ...
            new_inventories = state.inventories
    ...
    def name(self) -> str:
        """Environment name."""
        return "MarketEnv-InfiniteInventoryInfiniteEpisode"
